# App Store-tekst — Raak!

Alle velden voor App Store Connect, binnen de tekenlimieten. Kopieer per veld.
De app is meertalig; de Engelse en Franse metadata staan in
[appstore-tekst-en.md](appstore-tekst-en.md) en [appstore-tekst-fr.md](appstore-tekst-fr.md).
Stel Nederlands in als primaire taal van de app, en voeg English (U.S.) en
French (France) als extra lokalisaties toe.

---

## Naam (max 30 tekens)

> Raak!

*(5 tekens)*

## Ondertitel (max 30 tekens)

> Zeeslag voor het gezin

*(22 tekens — "zeeslag" en "gezin" tellen mee in de zoekindex en hoeven dus niet meer bij de trefwoorden)*

## Promotietekst (max 170 tekens, altijd aanpasbaar zonder review)

> Verstop je vloot, mik en roep: Raak! Samen aan één toestel of solo tegen de computer — zonder reclame en zonder account.

*(119 tekens)*

## Beschrijving (max 4000 tekens)

> Raak! is het klassieke zeeslagspel, gemaakt voor kinderen en het hele gezin. Schud je vloot, leg hem geheim klaar en schiet om de beurt op de zee van de ander. Raak? Dan mag je meteen nog een keer!
>
> SAMEN AAN ÉÉN TOESTEL
> Speel tegen elkaar op één iPhone of iPad. Het toestel gaat van hand tot hand, met een vriendelijk doorgeefscherm dat de zeeën geheim houdt — niemand kan stiekem spieken.
>
> OF TEGEN DE COMPUTER
> Drie tegenstanders met elk hun eigen karakter: Dommel schiet maar wat, Robbie mikt best goed en Professor Punt mist bijna nooit. En ze spelen eerlijk: de computer kent jouw vloot niet en zoekt écht, net als een mens. Zo groeit het spel mee met je kind.
>
> EEN VLOOT MET KARAKTER
> Vijf boten met een eigen naam en kleur, van de kleine Sloep tot het grote Kapiteinsschip. Drie zeeën om uit te kiezen: klein voor de jongsten, groot voor de echte kapiteins. Vloot leggen is één tik op de schudknop — slepen hoeft niet.
>
> GEMAAKT VOOR KINDERHANDEN
> Grote vakjes, vrolijke kleuren, een eigen avatar voor elke speler, een knal bij elke voltreffer en confetti voor de winnaar. Ondertussen wordt er heel wat gemikt, geteld en logisch geredeneerd — maar dat noemen we gewoon spelen.
>
> VEILIG EN RUSTIG
> Geen reclame, geen accounts, geen internet nodig. Alles blijft op je eigen toestel. Punt.
>
> GRATIS BEGINNEN, ÉÉN KEER ONTGRENDELEN
> Gratis speel je met z'n tweeën aan één toestel of solo tegen Robbie, in het klassieke thema. De eenmalige Gezinsversie ontgrendelt alle drie de tegenstanders, de kleurenthema's Snoep, Oceaan en Nacht, en statistieken per speler — met trofeeën, records en winreeksen. Eén aankoop, via Delen met gezin voor het hele gezin. Achter een ouder-poort, zodat kleine vingers niet per ongeluk iets kopen.
>
> Klaar om te schieten? Raak!

*(± 1600 tekens — ruim binnen de limiet)*

## Trefwoorden (max 100 tekens, gescheiden door komma's, geen spaties)

> zeeslagje,bootjes,schepen,kinderen,kinderspel,familie,gezelschapsspel,bordspel,samen,zeeslagspel

*(96 tekens)*

## In-app-aankoop

| Veld | Waarde |
|---|---|
| Referentienaam | Gezinsversie |
| Product-id | `com.pivo7.raak.gezin` |
| Type | Niet-verbruiksartikel, **gezinsdeling aan** |
| Weergavenaam (max 30) | Gezinsversie |
| Beschrijving (max 45) | Alle tegenstanders, thema's en statistieken |
| Prijs | €4,99 (prijspunt zelf te kiezen) |

## Overige velden in App Store Connect

- **Categorie:** primair *Games → Gezelschapsspellen (Board)*, secundair *Games → Gezin (Family)*. Wil je in de Kindercategorie: vink "Made for Kids" aan, leeftijdsband 6-8 — de ouder-poort en "geen gegevens"-privacy voldoen daaraan.
- **Leeftijdsclassificatie:** alle vragen "Nee" → 4+.
- **Privacylabel:** "Er worden geen gegevens verzameld."
- **Privacybeleid-URL:** publieke URL van `privacyverklaring.md` (bv. pivo7.be/raak/privacy).
- **Copyright:** © 2026 PIVO7.
- **Versietekst "Nieuw in deze versie" (eerste release):**
  > Welkom bij Raak! De allereerste versie: verstop je vloot, schiet om de beurt en zink de vloot van de ander — aan één toestel of tegen de computer.
