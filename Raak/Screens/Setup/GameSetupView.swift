...
struct GameSetupView: View {
    ...
    private func boardButton(_ size: BoardSize) -> some View {
        let picked = boardSize == size
        return Button {
            boardSize = size
        } label: {
            VStack(spacing: 3) {
        ...
        .accessibilityLabel("\(size.title), \(size.subtitle)")
        .accessibilityAddTraits(picked ? .isSelected : [])
    }

    /// De spelvorm: klassiek doorschieten op een treffer, of Salvo met een
    /// vaste reeks schoten per beurt.
    @ViewBuilder
    ...
    private func begin(with profiles: [PlayerProfile]) {
        gameStore.clear()
        let engine = GameEngine(mode: mode, variant: variant, profiles: profiles, boardSize: boardSize)
