...
/// staat op het hoogste podiumblok onder een draaiende stralenkrans, en de
/// winnende score telt op in flipcijfers. Choreografie: blokken springen
...
struct GameResultOverlay: View {
    ...
    private var shownCount: Int { animatesIn ? countedValue : winnerHits }
    ...
    private var winnerHits: Int {
        guard let winner, let index = players.firstIndex(where: { $0.id == winner.id }),
              hitCounts.indices.contains(index) else { return 0 }
    ...
    /// Spelers op podiumvolgorde: winnaar in het midden, de rest op treffers.
    /// Indeling links → rechts: [2e, 1e, 3e, 4e], zoals op een echt podium.
    private var podium: [(player: GamePlayer, rank: Int, hits: Int, sunk: Int)] {
        ...
        VStack(spacing: m.gutter * 0.85) {
            ...
            if let winner {
                if winner.isComputer {
                    ...
                    TallyTiles(value: shownCount, label: String(localized: "treffers"))
                        .opacity(bannerIn ? 1 : 0)
                        .accessibilityLabel(String(localized: "\(winnerHits) treffers"))
                }
            } else {
                Text(message)
                    .font(AppTheme.rounded(m.bodySize + 2, .bold))
    ...
    private func podiumColumn(_ entry: (player: GamePlayer, rank: Int, hits: Int, sunk: Int)) -> some View {
        ...
        return VStack(spacing: m.gutter * 0.6) {
            ...
            PodiumBlock(
                ...
                height: height,
                statText: String(localized: "\(entry.hits) raak · \(entry.sunk) boten")
            )
        ...
        .accessibilityLabel(
            isFirst
                ? String(localized: "\(entry.player.name), \(entry.hits) treffers, \(entry.sunk) boten gezonken, winnaar")
                : String(localized: "\(entry.player.name), \(entry.hits) treffers, \(entry.sunk) boten gezonken")
        )
    }
