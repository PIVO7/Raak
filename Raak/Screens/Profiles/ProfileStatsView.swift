...
/// De statistiekenpagina van één profiel: cijfers, het treffergrafiekje van
/// de laatste potjes en de trofeeënkast. Zonder Gezinsversie blijft het bij een
...
struct ProfileStatsView: View {
                ...
                if entitlements.isFamilyUnlocked {
                    ...
                    section("CIJFERS") {
                        VStack(spacing: m.gutter * 0.6) {
                            statRow(String(localized: "Gespeeld"), "\(profile.gamesPlayed)", icon: "water.waves")
    ...
    private func historyChart(for profile: PlayerProfile) -> some View {
        let games = Array(profile.history.suffix(10))
        let top = max(games.map(\.hits).max() ?? 1, 1)
        let barMaxHeight = m.avatarSize * 1.9

        return Group {
            if games.isEmpty {
                Text("Speel een potje en zie de grafiek hier groeien!")
                    ...
                    ForEach(Array(games.enumerated()), id: \.offset) { index, game in
                        VStack(spacing: 4) {
                            ...
                            RoundedRectangle(cornerRadius: m.cellCorner, style: .continuous)
                                .fill(game.won ? AppTheme.amber : AppTheme.sky)
                                .overlay {
                                    RoundedRectangle(cornerRadius: m.cellCorner, style: .continuous)
                                        .strokeBorder(AppTheme.ink, lineWidth: m.thinBorder)
                                }
                                // Nooit helemaal plat: ook een mager potje
                                // verdient een zichtbaar blokje.
                                .frame(height: max(barMaxHeight * CGFloat(game.hits) / CGFloat(top), 10))
                        }
                        .frame(maxWidth: .infinity)
                        .accessibilityElement(children: .ignore)
                        .accessibilityLabel(
                            game.won
                                ? String(localized: "Potje \(index + 1): \(game.hits) treffers, gewonnen")
                                : String(localized: "Potje \(index + 1): \(game.hits) treffers")
                        )
                    }
                }
                .padding(m.gutter)
    ...
    private func lockedTeaser(for profile: PlayerProfile) -> some View {
        ...
        return VStack(spacing: m.gutter) {
            ...
            VStack(spacing: m.gutter * 0.3) {
                ...
                Text("Trofeeën, het grafiekje en de gezinsrecords horen bij de Gezinsversie.")
                    .font(AppTheme.rounded(m.captionSize + 2, .bold))
