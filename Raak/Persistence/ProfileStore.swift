...
final class ProfileStore {
    ...
    private let logger = Logger(subsystem: "com.pivo7.raak", category: "profielen")

    init(filename: String = "raak-profiles.json") {
    ...
    /// deelnemers bij: potjes, winst, winreeks en de beste
    /// vangst. `hitCounts` is het aantal rake schoten per deelnemer, in
    /// dezelfde volgorde als `players` — ook de verliezer kan immers zijn
    /// persoonlijke record verbeteren.
    func recordGameResult(players: [GamePlayer], winnerProfileIDs: [UUID], hitCounts: [Int]) {
        for (playerIndex, player) in players.enumerated() where !player.isComputer {
